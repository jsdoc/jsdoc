...
Narcissus.lexer = (function() {
    ...
    Tokenizer.prototype = {
                ...
                if (ch === '\n' && !this.scanNewlines) {
                    ...
                    for (;;) {
                        ...
                        if (ch === '*') {
                            ...
                            if (next === '/') {
                                if (jsdocComment) this.jsdocs.push(jsdocComment);
                                jsdocComment = null;
                                
                                this.cursor++;
                                if (typeof input[this.cursor+1] === 'undefined') { // eof
                                    if (typeof this.onJsDoc === 'function') {
                                        while(this.jsdocs.length) {
                                            this.onJsDoc(this.jsdocs.shift());
                                        }
                                    }
                                }
                                break;
                        ...
                        if (typeof jsdocComment === 'string') { jsdocComment += ch; }
                    }
                } else if (ch === '/' && next === '/') {
                    this.cursor++;
                    for (;;) {
        ...
        get: function (scanOperand) {
            ...
            if (this.jsdocs && this.jsdocs.length) {
                while (this.jsdocs.length > 1) { // orphaned jsdocComments have no code token
                    if (typeof this.onJsDoc === 'function' && this.jsdocs.length) {
                        this.onJsDoc(this.jsdocs.shift());
                    }
                    else { // discard?
                        this.jsdocs.shift();
