...
var test = module.exports = {
    'Basic tests.': {
        ...
        'The require function should take one required string argument.': function(t) {
            t.expect(3);
            t['throws']( function() { require(); });
            t['throws']( function() { require(1); } );
            t['throws']( function() { require(''); } );
            t.done();
        }
    },
...
function toAbsolute(relPath) {
	var absPath = String( new java.io.File(relPath).getAbsolutePath() )
		.replace(/(\/|\\)[^\/\\]+\/\.\.(\/|\\)/g, SLASH)
		.replace(/(\/|\\)\.(\/|\\|$)/g, SLASH);
    return absPath;
}
