...
function parseType({text, originalTitle}, {canHaveName, canHaveType}, meta) {
    try {
        ...
        if (meta.filename) {
            sourceInfo += ` for source file ${path.join(meta.path, meta.filename)}`;
            if (meta.lineno) {
                sourceInfo += ` in line ${meta.lineno}`;
            }
        }

        logger.error(
            `Unable to parse a tag's type expression${sourceInfo} with tag title ` +
            `"${originalTitle}" and text "${text}": ${e.message}`
        );
...
class Tag {
    ...
    constructor(tagTitle, tagBody = '', meta = {}) {
        ...
        tagBody = String(tagBody);

        this.originalTitle = trim(tagTitle);

        /** The title of the tag (for example, `title` in `@title text`). */
        this.title = jsdoc.tag.dictionary.normalize(this.originalTitle);
         ...
         * removed.
         * + If the tag's `keepsWhitespace` option is set to `true`, leading and trailing whitespace
         * are not trimmed, unless the `removesIndent` option is also enabled.
         * + If the tag's `removesIndent` option is set to `true`, any indentation that is shared by
         * every line in the string is removed. This option is ignored unless `keepsWhitespace` is
         * set to `true`.
         *
         * **Note**: If the tag text is the name of a symbol, and the symbol's name includes leading
         * or trailing whitespace (for example, the property names in
         * `{ ' ': true, ' foo ': false }`), the tag text is not trimmed. Instead, the tag text is
         * wrapped in double quotes to prevent the whitespace from being trimmed.
         */
