...
var nodeToString = exports.nodeToString = function(node) {
    ...
    switch (node.type) {
        ...
        // TODO: why convert the value to a string?
        case Syntax.Literal:
        ...
        default:
            // TODO: why not return null (or throw) if we don't recognize the node type?
            str = Syntax[node.type] || '';
    }
...
var getInfo = exports.getInfo = function(node) {
    ...
    switch (node.type) {
        ...
        case Syntax.FunctionDeclaration:
            ...
            // TODO: use info.node.type instead of 'function'
        ...
        case Syntax.Property:
            // TODO: can we just put 'node' here?
            info.node = node.value;
            info.name = nodeToString(node.key);
            info.value = nodeToString(info.node);

            if ( isAccessor(node) ) {
                // TODO: if we make the above change, need to pass info.node.value
                info.type = nodeToString(info.node);
        ...
        case Syntax.VariableDeclarator:
            // TODO: can we just put 'node' here and adjust info.type/info.value accordingly?
            info.node = node.init || node.id;
            info.name = node.id.name;
            // TODO: can we pass null if there's no init property?
            info.type = info.node.type || 'undefined';
            // TODO: fix for vars with no init property
            info.value = nodeToString(info.node);
            break;

        default:
            // TODO: why not return null (or throw) if we don't recognize the node type?
            info.name = nodeToString(node) || null;
            // TODO: info.type?
    }
