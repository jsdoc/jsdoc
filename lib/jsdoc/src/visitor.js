...
const getBasename = jsdoc.name.getBasename;
...
exports.Visitor = class Visitor {
    ...
    visitNodeComments(node, parser, filename) {
        ...
        if (node.type === Syntax.ExpressionStatement && node.directive === 'use strict' &&
            ...
            // to be safe, we verify that the trailing comments came after the next node in the
            // Program body, which means the comments were attached to the wrong node
