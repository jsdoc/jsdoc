...
function makeRestParamFinisher(parser) {
    return function(e) {
        var documentedParams;
        var parentDoclet = getParentDocletFromEvent(parser, e);

        if (!parentDoclet) {
            return;
        }

        documentedParams = parentDoclet.params = parentDoclet.params || [];
        for (var i = documentedParams.length - 1; i >= 0; i--) {
            if (documentedParams[i].name === e.code.node.name) {
...
Visitor.prototype.makeSymbolFoundEvent = function(node, parser, filename) {
    ...
    switch (node.type) {
        ...
        // This is an extremely common type of node; we only care about a) function parameters with
        // inline comments and b) rest parameters. No need to fire an event in other cases.
        case Syntax.Identifier:
            ...
            // rest parameters, with or without comments
            // TODO: move this to FunctionExpression so we only do this once per function
            else if (parent && parent.rest === node) {
                extras.finishers = [makeRestParamFinisher(parser)];
                e = new SymbolFound(node, filename, extras);
