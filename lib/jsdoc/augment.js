    ...
    Object.keys(index).forEach(function(name) {
        ...
        for (var i = 0, ii = doclets.length; i < ii; ++i) {
            ...
            if (kinds.indexOf(doc.kind) !== -1) {
                dependencies[name] = {};
                len = doc[propertyName] && doc[propertyName].length || 0;
                for (var j = 0; j < len; ++j) {
...
function reparentDoclet(parent, child) {
    ...
    child.longname = name.combineName(parts);
}
...
function staticToInstance(doclet) {
    ...
    parts.scopePunc = name.SCOPE.PUNC.INSTANCE;
    doclet.longname = name.combineName(parts);
