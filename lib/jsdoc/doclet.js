...
const Tag = require('jsdoc/tag').Tag;

const applyNamespace = jsdoc.name.applyNamespace;
const atomize = jsdoc.name.atomize;
const DEFAULT_SCOPE = jsdoc.name.SCOPE.NAMES.STATIC;
const hasLeadingScope = jsdoc.name.hasLeadingScope;
const hasOwnProp = Object.prototype.hasOwnProperty;
const hasTrailingScope = jsdoc.name.hasTrailingScope;
const isFunction = jsdoc.src.astnode.isFunction;
const LONGNAMES = jsdoc.name.LONGNAMES;
const MODULE_NAMESPACE = jsdoc.name.MODULE_NAMESPACE;
const nameIsLongname = jsdoc.name.nameIsLongname;
const prototypeToPunc = jsdoc.name.prototypeToPunc;
const puncToScope = jsdoc.name.puncToScope;
const SCOPE = jsdoc.name.SCOPE;
const scopeToPunc = jsdoc.name.scopeToPunc;
...
function resolve(doclet) {
    ...
    if (memberof || doclet.forceMemberof) { // @memberof tag given
        ...
        if (name && nameIsLongname(name, memberof) && name !== memberof) {
            about = atomize(name, (doclet.forceMemberof ? memberof : undefined));
        }
        ...
        // like @memberof foo# or @memberof foo~
            ...
            about = atomize(memberof + name, (doclet.forceMemberof ? memberof : undefined));
        }
        else if (name && doclet.scope) {
            about = atomize(memberof + (scopeToPunc[doclet.scope] || '') + name,
...
function combineWithLogic(primary, secondary, target, include) {
    include.forEach(property => {
        ...
        if (hasOwnProp.call(primary, property)) {
        ...
        else if (hasOwnProp.call(secondary, property)) {
...
class Doclet {
    ...
    // TODO(hegemonic): We call this method in the constructor _and_ in `jsdoc/src/handlers`. It
    // appears that if we don't call the method twice, various doclet properties can be incorrect,
    // including name and memberof.
    ...
    addTag(title, text) {
        ...
        jsdoc.tag.validator.validate(newTag, tagDef, this.meta);
    ...
    setMemberof(sid) {
        ...
        this.memberof = removeGlobal(sid)
            // TODO(hegemonic): use prototypeToPunc instead?
    ...
    setScope(scope) {
        ...
        if (!scopeNames.includes(scope)) {
            ...
            errorMessage = `The scope name ${scope} is not recognized. Use one of the following ` +
                `values: ${scopeNames}`;
            if (filepath) {
