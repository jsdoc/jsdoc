...
Dictionary.prototype.getContainerTagNames = function() {
    var containers = [];
    var exceptions = ['constructor']

    for(tag in this._tags) {
        var containerName = this._tags[tag].containerName;
        if (containerName !== undefined && containers.indexOf(containerName) === -1) {
            containers.push(
              {
                kind: tag,
