...
function getTagInfo(tagValue, canHaveName, canHaveType) {
    ...
    if (canHaveName) {
        nameAndDescription = jsdoc.name.splitNameAndDescription(text);
...
function getTypeStrings(parsedType, isOutermostType) {
    ...
    switch (parsedType.type) {
        ...
        case TYPES.TypeApplication:
            ...
            if (isOutermostType) {
                applications = parsedType.applications.map(
                    application => catharsis.stringify(application)
                ).join(', ');
                typeString = `${getTypeStrings(parsedType.expression)}.<${applications}>`;
