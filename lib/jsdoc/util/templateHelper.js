...
var containerCache = [];
var containers = exports.containers = function () {
    if(containerCache.length === 0) {
        containerCache = dictionary.getContainerTagNames();
    }
...
exports.getMembers = function(data) {
    ...
    for(var i = 0; i < containers().length; i++) {
        var s = (containers()[i][containers()[i].length - 1] === 's') ? "es" : "s";
        members[containers()[i] + s] = find( data, {kind: containers()[i]});
    }
