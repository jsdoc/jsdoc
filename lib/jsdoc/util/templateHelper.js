...
exports.addEventListeners = function(data) {
    ...
    var listeners = find(data, function (d) { return d && d.length; });

    if (!listeners.length || !events.length) {
        return;
    }
    ...
    listeners.forEach(function (listener, i, arr) {
        l = listener.listens;
        l.forEach(function (eventLongname) {
            doc = _events[eventLongname] || find(data, {longname: eventLongname})[0];
            if (doc) {
                if (!doc.listeners) {
