...
var containerCache = dictionary.getContainerTagNames();
var containers = exports.containers = containerCache;

function containerHasKind(kind) {
    var hasKind = false;
...
exports.getMembers = function(data) {
    ...
    containers.forEach(function(container) {
        members[container.container] = find( data, {kind: container.kind});
    });
...
exports.createLink = function(doclet) {
    ...
    if (containerHasKind(doclet.kind) === false) {
        match = /(\S+):/.exec(longname);
        if (match && containerHasKind(doclet.kind)) {
            fakeContainer = match[1];
        }
    }

    // the doclet gets its own HTML file
    if ( containerHasKind(doclet.kind) || isModuleExports(doclet) ) {
        filename = getFilename(longname);
    }
    // mistagged version of a doclet that gets its own HTML file
    else if ( containerHasKind(doclet.kind) === false && fakeContainer ) {
