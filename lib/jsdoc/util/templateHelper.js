...
exports.resolveLinks = function(str) {
    var linkRegExp = /(?:\[(.+?)\])?\{@(link(?:plain|code)?)\s+(.+?)\}/gi;
    var tutorialRegExp = /(?:\[(.+?)\])?\{@tutorial\s+(.+?)\}/gi;

    function processLinkTag(match, linkText, tag, text) {
        var monospace;
        var split;
        var target;
        ...
        return buildLink(target, linkText, {
            linkMap: longnameToUrl,
            monospace: monospace
        });
    }

    function processTutorialTag(match, linkText, tutorial) {
        return toTutorial(tutorial, linkText);
    }

    return str.replace(linkRegExp, processLinkTag)
        .replace(tutorialRegExp, processTutorialTag);
};
